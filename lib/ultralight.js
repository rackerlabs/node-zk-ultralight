...
ZkCxn.prototype._changeState = function(to, data) {
  ...
  log.trace1f('ZkCxn.changeState', { from: this._cxnState, to: to, data: data });
  this._cxnState = to;
...
ZkCxn.prototype.lock = function(name, txnId, callback) {
  ...
  this.onConnection(function locking(err) {
    var lockpath = name.slice(0, name.lastIndexOf('/') + 1);
    if (err) {
      callback(err);
      return;
    }
    ...
    async.auto({
      // ensure the parent path exists, (client doesn't like paths ending in /)
      'path': self._zk.mkdirp.bind(self._zk, lockpath.slice(0, -1)),
      // create a node in the lock queue
      'node': ['path', self._zk.create.bind(self._zk, name, txnId, zookeeper.CreateMode.EPHEMERAL_SEQUENTIAL)],
...
ZkCxn.prototype._negotiateLock = function(name, results, callback) {
  ...
  async.until(function() { return locked; }, function(callback) {
    callback = _.once(callback);
    self._zk.getChildren(lockpath.slice(0, -1), function(err, children) {
      if (err) {
...
ZkCxn.prototype.close = function(callback) {
  ...
  switch (this._cxnState) {
    ...
    case this.cxnStates.CLOSED:
      ...
      break;
    case this.cxnStates.CONNECTING:
      // it is unclear if we should wait for connection to close, don't think so
      self._zk.close();
      this._zk = null;
      self._changeState(self.cxnStates.CLOSED);
      break;
  }
};
